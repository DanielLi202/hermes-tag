...
完整安装与线上验证：见 [after-install.md](after-install.md)。Slack 接入：见 [docs/slack-setup.md](docs/slack-setup.md)。钉钉接入与能力边界：见 [docs/dingtalk.md](docs/dingtalk.md)。遇到疑似故障？先查 [docs/known-limits.zh-CN.md](docs/known-limits.zh-CN.md)（英文）——它把常见症状对应到飞书平台 / hermes 核心的限制，这些不是本插件能修的。

## 使用

在群里，`/tag` 命令需要 @ 机器人。

- `/tag status` →（仅管理员）返回适配器平台、`tier0_full_ingest` / `l2_context` / `tier1_at_memory` 等能力标志，以及当前按群统计的指标。
